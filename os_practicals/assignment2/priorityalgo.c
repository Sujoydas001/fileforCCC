...
void heapify( minHeap *heap , int idx ){
    ...
    if( l_child < heap->size && heap->heap[parent].priority > heap->heap[l_child].priority ){
        parent = l_child  ; 
    }
    if( r_child < heap->size && heap->heap[parent].priority > heap->heap[r_child].priority ){
        parent = r_child  ; 
    }
    if( parent != idx ){
...
void insert( minHeap *heap , process *data ){
    ...
    heap->heap[heap->size].priority = data->priority ; 
...
int main(){
    ...
    int n  , pid_ref , b_time_ref ,priority_ref; 
    ...
    for( int i = 0 ; i < n ; i++ ){
        fscanf(ptr,"%d%d%d",&pid_ref , &b_time_ref , &priority_ref ) ; 
        refProcess->pid = pid_ref ; 
        refProcess->b_time = b_time_ref ; 
        refProcess->priority = priority_ref ; 
